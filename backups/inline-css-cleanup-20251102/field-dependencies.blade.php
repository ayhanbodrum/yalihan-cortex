...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.property-type-manager.show', $kategori->id) }}"
               class="inline-flex items-center px-4 py-2 bg-gray-600 text-white font-semibold rounded-lg shadow-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 active:scale-95 dark:bg-gray-800 dark:hover:bg-gray-600">
    ...
    <div class="bg-gray-50 dark:bg-gray-800 rounded-xl shadow-lg p-6">
            ...
            <div class="flex gap-2">
                <select style="color-scheme: light dark;" id="filterYayinTipi" class="text-sm max-w-xs px-4 py-2 border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
        ...
        <div class="overflow-x-auto">
            @foreach($fieldDependencies as $yayinTipi => $fields)
                @php
                    // ⚠️ Filtreleme: Belirli yayın tiplerini gösterme
                    $excludedYayinTipleri = ['Devren Satılık', 'Günlük Kiralık', 'Satılık'];
                    if (in_array($yayinTipi, $excludedYayinTipleri)) {
                        continue;
                    }
                @endphp
                ...
                <div class="space-y-2" id="fields-{{ Str::slug($yayinTipi) }}">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors field-row"
                        ...
                        <div class="flex items-center gap-4 flex-1">
                            <div class="flex items-center justify-center w-10 h-10 bg-gray-50 dark:bg-gray-800 rounded-lg text-xl">
                                {{ $field->field_icon ?? '📋' }}
                            </div>
                            <div class="flex-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="field-name-container flex items-center gap-2 flex-1">
                                        ...
                                        <input type="text"
                                               class="hidden field-name-input px-3 py-1 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
                                               data-field-id="{{ $field->id }}"
                                    ...
                                    @if($field->field_unit)
                                    <span class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white text-xs rounded-full">
                                        {{ $field->field_unit }}
                                    </span>
                                    @endif
                                    @if($field->required)
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer"
                                ...
                                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-300 dark:peer-focus:ring-green-800 rounded-full peer dark:bg-gray-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
                            ...
                            <button onclick="openEditFieldModal({{ $field->id }}, {{ json_encode($field) }})"
                                    class="px-3 py-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 border border-blue-200 dark:border-blue-800 rounded-lg text-sm flex items-center gap-2"
                                    title="Düzenle">
...
<div id="addFieldModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="bg-gray-50 dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <!-- Modal Header -->
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-600">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">
                ➕ Yeni Alan Ekle
            </h3>
            <button onclick="closeAddFieldModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
        ...
        <form action="{{ route('admin.property-type-manager.field-dependencies.store', $kategori->id) }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label for="yayin_tipi" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Yayın Tipi <span class="text-red-500">*</span>
                    </label>
                    <select style="color-scheme: light dark;" name="yayin_tipi" id="yayin_tipi" required
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-500 dark:bg-gray-600 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200">
                ...
                <div>
                    ...
                    <input type="text" name="field_name" id="field_name" required
                           ...
                           class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-500 dark:bg-gray-600 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent">
                </div>

                <!-- Field Slug -->
                <div>
                    <label for="field_slug" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                ...
                <!-- Field Type -->
                <div>
                    <label for="field_type" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Alan Tipi <span class="text-red-500">*</span>
                    </label>
                    <select style="color-scheme: light dark;" name="field_type" id="field_type" required
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200">
                        <option value="">Seçiniz...</option>
                ...
                <!-- Field Category -->
                <div>
                    <label for="field_category" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Alan Kategorisi <span class="text-red-500">*</span>
                    </label>
                    <select style="color-scheme: light dark;" name="field_category" id="field_category" required
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200">
                        <option value="">Seçiniz...</option>
                ...
                <!-- Field Icon -->
                <div>
                    <label for="field_icon" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        İkon (Emoji)
                    </label>
                ...
                <!-- Field Unit -->
                <div>
                    <label for="field_unit" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Birim
                    </label>
                ...
                <div class="md:col-span-2" id="field_options_wrapper" style="display: none;">
                    <label for="field_options" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Seçenekler (JSON formatında)
                    </label>
                    <textarea name="field_options" id="field_options" rows="3"
                              placeholder='["1+1", "2+1", "3+1"]'
                              class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-500 dark:bg-gray-600 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent"></textarea>
                ...
                <div class="md:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="enabled" value="1" checked
                               class="rounded text-green-600 focus:ring-blue-500 dark:focus:ring-blue-400 mr-2">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="required" value="1"
                               class="rounded text-green-600 focus:ring-blue-500 dark:focus:ring-blue-400 mr-2">
                        <span class="text-sm text-gray-900 dark:text-white">Zorunlu</span>
                    </label>

                    <label class="flex items-center">
                        <input type="checkbox" name="searchable" value="1"
                               class="rounded text-green-600 focus:ring-blue-500 dark:focus:ring-blue-400 mr-2">
                        <span class="text-sm text-gray-900 dark:text-white">Aranabilir</span>
                    </label>

                    <label class="flex items-center">
                        <input type="checkbox" name="show_in_card" value="1"
                               class="rounded text-green-600 focus:ring-blue-500 dark:focus:ring-blue-400 mr-2">
                        <span class="text-sm text-gray-900 dark:text-white">Kartta Göster</span>
                    </label>

                    <label class="flex items-center">
                        <input type="checkbox" name="ai_auto_fill" value="1"
                               class="rounded text-green-600 focus:ring-blue-500 dark:focus:ring-blue-400 mr-2">
                        <span class="text-sm text-gray-900 dark:text-white">AI Doldurma</span>
                    </label>

                    <label class="flex items-center">
                        <input type="checkbox" name="ai_suggestion" value="1"
                               class="rounded text-green-600 focus:ring-blue-500 dark:focus:ring-blue-400 mr-2">
                        <span class="text-sm text-gray-900 dark:text-white">AI Öneri</span>
                    </label>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <button type="button" onclick="closeAddFieldModal()" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white font-semibold rounded-lg shadow-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 active:scale-95 dark:bg-gray-800 dark:hover:bg-gray-600">
...
<div id="editFieldModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-600">
            ...
            <button onclick="closeEditFieldModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                <i class="fas fa-times text-2xl"></i>
            </button>
        </div>
        ...
        <form id="editFieldForm" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label for="edit_yayin_tipi" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Yayın Tipi <span class="text-yellow-500" title="Bu alan değiştirilemez">🔒</span>
                    </label>
                    <select style="color-scheme: light dark;" id="edit_yayin_tipi" disabled
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-500 bg-gray-100 dark:bg-gray-600 text-gray-900 dark:text-white rounded-lg cursor-not-allowed opacity-80 transition-all duration-200">
                        @foreach($yayinTipleri as $yt)
                            @php
                                // ⚠️ Filtreleme: Belirli yayın tiplerini gösterme
                                $excludedYayinTipleri = ['Devren Satılık', 'Günlük Kiralık', 'Satılık'];
                ...
                <div>
                    <label for="edit_field_name" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Alan Adı <span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    <label for="edit_field_slug" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Alan Slug <span class="text-yellow-500" title="Bu alan değiştirilemez">🔒</span>
                    </label>
                    <input type="text" id="edit_field_slug" readonly
                           class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-500 bg-gray-100 dark:bg-gray-600 text-gray-900 dark:text-white rounded-lg cursor-not-allowed opacity-80">
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        <i class="fas fa-lock mr-1"></i> Bu alan kimlik bilgisidir ve değiştirilemez
                    </p>
                </div>
                ...
                <div>
                    <label for="edit_field_type" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Alan Tipi <span class="text-red-500">*</span>
                    </label>
                    <select style="color-scheme: light dark;" name="field_type" id="edit_field_type" required
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200">
                        <option value="text">Metin (text)</option>
                        <option value="number">Sayı (number)</option>
                        <option value="boolean">Evet/Hayır (boolean)</option>
                ...
                <div>
                    <label for="edit_field_category" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Alan Kategorisi <span class="text-red-500">*</span>
                    </label>
                    <select style="color-scheme: light dark;" name="field_category" id="edit_field_category" required
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200">
                        <option value="fiyat">💰 Fiyat</option>
                        <option value="ozellik">⭐ Özellik</option>
                        <option value="dokuman">📄 Döküman</option>
                ...
                <div>
                    <label for="edit_field_icon" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        İkon
                    </label>
                ...
                <div>
                    <label for="edit_field_unit" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Birim
                    </label>
                ...
                <div>
                    <label for="edit_order" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Sıra
                    </label>
                ...
                <div id="edit_field_options_wrapper" class="md:col-span-2" style="display: none;">
                    <label for="edit_field_options" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">
                        Seçenekler (JSON)
                    </label>
...
/* Sürüklenirken ghost (placeholder) */
.sortable-ghost {
    opacity: 0.4;
...
/* Seçilen element (aktif sürüklenen) */
.sortable-chosen {
...
<script>
// 🔥 DRAG & DROP SIRALAMASI - Sortable.js
document.addEventListener('DOMContentLoaded', function() {
    // Her yayın tipi için ayrı sortable instance
    document.querySelectorAll('.yayin-tipi-group .space-y-2').forEach(container => {
        new Sortable(container, {
            animation: 150,
            handle: '.drag-handle', // Sadece drag handle'dan sürüklenebilir
...
function updateFieldOrder(fieldOrders) {
    fetch("{{ route('admin.property-type-manager.update-field-order') }}", {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            fields: fieldOrders // ✅ Backend 'fields' bekliyor
        })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            window.toast?.success('✅ Sıralama güncellendi!');
            console.log('📊 Updated field orders:', data.data);
        } else {
            window.toast?.error('Hata: ' + (data.message || 'Sıralama güncellenemedi'));
            console.error('❌ Order update failed:', data);
        }
    })
    .catch(error => {
        console.error('❌ Order update error:', error);
        window.toast?.error('Bağlantı hatası! Sıralama kaydedilemedi.');
    });
}
