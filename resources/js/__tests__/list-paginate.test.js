...
describe('ListPaginate per_page precedence', () => {
  it('URL per_page has priority over localStorage and default', async () => {
    ...
    window.ApiAdapter = { get: vi.fn().mockResolvedValue({ data: [], meta: { total: 0, current_page: 1, last_page: 1, per_page: 50 } }) }
    // Auto-init runs at import; but we call init explicitly for reliability
    await ListPaginateModule.init({ endpoint: '/api/admin/api/v1/kisiler', containerSelector: 'table tbody', paginateSelector: '.mt-6', perPageKey: 'yalihan_admin_per_page', render: () => '' })
  ...
  it('localStorage used when URL missing', async () => {
    ...
    window.ApiAdapter = { get: vi.fn().mockResolvedValue({ data: [], meta: { total: 0, current_page: 1, last_page: 1, per_page: 100 } }) }
    await ListPaginateModule.init({ endpoint: '/api/admin/api/v1/kisiler', containerSelector: 'table tbody', paginateSelector: '.mt-6', perPageKey: 'yalihan_admin_per_page', render: () => '' })
    await Promise.resolve()
    const select = document.querySelector('select[data-per-page-select]')
  ...
  it('default 20 when neither URL nor localStorage present', async () => {
    setupDOM()
    window.ApiAdapter = { get: vi.fn().mockResolvedValue({ data: [], meta: { total: 0, current_page: 1, last_page: 1, per_page: 20 } }) }
    await ListPaginateModule.init({ endpoint: '/api/admin/api/v1/kisiler', containerSelector: 'table tbody', paginateSelector: '.mt-6', perPageKey: 'yalihan_admin_per_page', render: () => '' })
    await Promise.resolve()
    await new Promise((r) => setTimeout(r, 0))
    const select = document.querySelector('select[data-per-page-select]')
...
describe('AbortController behavior', () => {
  it('only last request applies on rapid clicks', async () => {
    ...
    window.ApiAdapter = { get: vi.fn()
      .mockImplementationOnce(() => Promise.resolve({ data: [], meta: { total: 0, current_page: 1, last_page: 2, per_page: 20 } }))
      .mockImplementationOnce(() => Promise.resolve({ data: [], meta: { total: 1, current_page: 2, last_page: 2, per_page: 20 } }))
      .mockImplementationOnce(() => Promise.resolve({ data: ['second'], meta: { total: 1, current_page: 1, last_page: 2, per_page: 20 } }))
    }
    await ListPaginateModule.init({ endpoint: '/api/admin/api/v1/kisiler', containerSelector: 'table tbody', paginateSelector: '.mt-6', perPageKey: 'yalihan_admin_per_page', render: (items) => { document.querySelector('table tbody').innerHTML = items.length ? 'X' : 'Y'; return '' } })
