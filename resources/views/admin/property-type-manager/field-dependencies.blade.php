...
<div class="container mx-auto px-4 py-8" x-data="{
    activeTab: '{{ $yayinTipleri->first()->slug ?? $yayinTipleri->first()->yayin_tipi ?? '' }}',
    ...
    <div class="max-w-7xl mx-auto">
        ...
        @if($yayinTipleri->count() > 0)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg mb-6">
                <div class="border-b border-gray-200 dark:border-gray-700 px-6">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        @foreach($yayinTipleri as $index => $yayinTipi)
                            <button @click="activeTab = '{{ $yayinTipi->slug ?? $yayinTipi->yayin_tipi }}'"
                                :class="activeTab === '{{ $yayinTipi->slug ?? $yayinTipi->yayin_tipi }}' ? 'active' : ''"
                                ...
                                <span class="ml-2 px-2 py-1 text-xs rounded-full"
                                      :class="activeTab === '{{ $yayinTipi->slug ?? $yayinTipi->yayin_tipi }}' ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'">
                                    {{ $fieldDependencies[$yayinTipi->slug ?? $yayinTipi->yayin_tipi]->count() }}
                                </span>
                            </button>
                        @endforeach
                    </nav>
                ...
                <!-- Tab Contents -->
                @foreach($yayinTipleri as $index => $yayinTipi)
                    <div x-show="activeTab === '{{ $yayinTipi->slug ?? $yayinTipi->yayin_tipi }}'" 
                        ...
                        <div class="flex justify-between items-center mb-6">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    {{ $fieldDependencies[$yayinTipi->slug ?? $yayinTipi->yayin_tipi]->count() }} özellik atandı
                                </p>
                            </div>
                            <button @click="showAddFeatureModal = true; selectedPropertyTypeId = {{ $yayinTipi->id }}"
                                ...
                                @foreach($assignments as $assignment)
                                    ...
                                    <div class="feature-card bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 shadow-sm">
                                        ...
                                        <div class="flex items-start justify-between mb-4">
                                            ...
                                            <!-- Delete Button -->
                                            <button @click="unassignFeature({{ $yayinTipi->id }}, {{ $feature->id }})"
                                        ...
                                        <!-- AI Features (if any) -->
                                        @if($feature->hasAiCapabilities())
                                            <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                                                <div class="flex flex-wrap gap-1">
                                                    @if($feature->ai_auto_fill)
                                ...
                                <label class="flex items-center p-3 bg-gray-50 dark:bg-gray-900 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer transition-colors border border-transparent hover:border-blue-300 dark:hover:border-blue-700">
                                    ...
                                    <span class="ml-3 flex-1">
                                        <span class="font-medium text-gray-900 dark:text-white">
                                            @if($feature->field_icon){{ $feature->field_icon }} @endif
                                            {{ $feature->name }}
                                        </span>
