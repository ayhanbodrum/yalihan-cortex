                                ...
                                @if (config('app.debug'))
                                    <div class="mt-3 p-3 bg-gray-100 dark:bg-gray-900 rounded text-xs font-mono">
                                        ...
                                        <div class="text-gray-600 dark:text-gray-400 space-y-1">
                                            ...
                                            <div>Parent ID: <span
                                                    class="font-bold">{{ $kategori->parent_id ?? 'NULL' }}</span></div>
                ...
                <div class="mb-6 last:mb-0 border-b dark:border-gray-700 pb-6 last:border-0 last:pb-0">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        @foreach ($allYayinTipleri as $yayinTipi)
                            ...
                            <label
                                class="flex items-center p-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer {{ $active ? 'bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700' : 'bg-gray-50 dark:bg-gray-800 border-gray-300 dark:border-gray-600' }}">
                                <input type="checkbox" class="rounded mr-2 yayin-tipi-toggle"
                                    ...
                                    data-yayin-tipi-name="{{ $yayinTipi->yayin_tipi }}" {{ $active ? 'checked' : '' }}
                                    onchange="toggleYayinTipiRelation(this)">
                                <span
                                    class="text-sm font-medium text-gray-900 dark:text-white">{{ $yayinTipi->yayin_tipi }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            'yayin_tipi',
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <input type="checkbox" class="rounded field-dependency-toggle"
                                                ...
                                                {{ $status ? 'checked' : '' }} onchange="toggleFieldDependency(this)">
                                        </td>
                                    @endforeach
                                </tr>
                            @endforeach
    ...
    @push('scripts')
        <script data-timestamp="{{ time() }}">
            ...
            async function deleteYayinTipi(yayinTipiId, yayinTipiName) {
                ...
                try {
                    const url = '/admin/property-type-manager/{{ $kategori->id }}/yayin-tipi/' + yayinTipiId;
