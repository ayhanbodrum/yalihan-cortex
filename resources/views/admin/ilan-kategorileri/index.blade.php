    ...
    <div class="container mx-auto px-4 py-6" x-data="kategorilerManager()">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/30 rounded-lg p-3">
                ...
                <div class="text-xl font-bold text-green-700 dark:text-green-300">{{ $istatistikler['ana_kategoriler'] }}
                </div>
            </div>
            <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-lg p-3">
                ...
                <div class="text-xl font-bold text-amber-700 dark:text-amber-300">{{ $istatistikler['alt_kategoriler'] }}
                </div>
            </div>
            <div
                class="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800/30 rounded-lg p-3">
        ...
        <div
            ...
            <div class="p-6">
                ...
                <div x-show="selectedItems.length > 0" x-transition
                    class="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/30 rounded-lg">
                    <div class="flex items-center justify-between">
                ...
                <form method="GET" class="flex flex-wrap items-end gap-2 mb-4" @submit.prevent="submitFilters()">
                    <div class="flex-1 min-w-[200px] relative">
                        <input type="text" id="search" name="search" value="{{ request('search') }}"
                            ...
                            class="w-full px-4 py-2.5 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all duration-200 hover:border-blue-400"
                    ...
                    <select style="color-scheme: light dark;" id="seviye" name="seviye"
                        class="px-4 py-2.5 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-all duration-200 cursor-pointer hover:border-blue-400"
                        x-model="filters.seviye" @change="applyFilters()">
                    ...
                    <button type="button" @click="clearFilters()"
                        class="px-3 py-1.5 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg" :disabled="loading">
                        Temizle
                    </button>
                </form>
                @push('scripts')
                    <script>
                        window._kategoriAdlari = [
                            @foreach ($kategoriler as $kategori)
                                @if ($kategori->name)
                                    "{{ addslashes($kategori->name) }}",
                                @endif
                            @endforeach
                        ];
                            ...
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach ($kategoriler as $kategori)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                                        ...
                                        <td class="px-6 py-4">
                                            <input type="checkbox" id="kategori-{{ $kategori->id }}"
                                                value="{{ $kategori->id }}" class="rounded"
                                                @change="toggleItemSelection({{ $kategori->id }})">
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <span
                                                    class="text-sm font-medium text-gray-900 dark:text-white">{{ $kategori->name }}</span>
                                                <span
                                                    class="px-2 py-1 text-xs rounded-full {{ $kategori->seviye == 0 ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                class="text-sm text-gray-500 dark:text-gray-400">{{ $kategori->parent?->name ?? '—' }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $kategori->status ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' }}">
