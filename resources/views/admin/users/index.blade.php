        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @if(count($duplicateEmails)>0)
                    <span class="px-3 py-1.5 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400 text-xs font-semibold rounded-full">⚠️ Mükerrer: {{ count($duplicateEmails) }}</span>
                @else
                ...
                @endif
                <span class="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400 text-xs font-semibold rounded-full">💤 Pasif: {{ $passiveCount }}</span>
            </div>
        </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.kullanicilar.index') }}" 
                       class="px-3 py-1.5 rounded-lg text-sm font-medium transition-all
                              {{ !request('status') ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                        Tümü
                    </a>
                    <a href="{{ route('admin.kullanicilar.index', ['status' => '1']) }}" 
                       class="px-3 py-1.5 rounded-lg text-sm font-medium transition-all
                              {{ request('status') == '1' ? 'bg-green-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                        Aktif
                    </a>
                    <a href="{{ route('admin.kullanicilar.index', ['status' => '0']) }}" 
                       class="px-3 py-1.5 rounded-lg text-sm font-medium transition-all
                              {{ request('status') == '0' ? 'bg-red-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                        Pasif
                    </a>
                </div>
                ...
                <form method="GET" class="flex items-center gap-2">
                    ...
                    <select style="color-scheme: light dark;" name="role" onchange="this.form.submit()"
                        ...
                        <option value="danisman" {{ request('role')=='danisman' ? 'selected' : '' }}>Danışman</option>
                    ...
                    @forelse($users as $user)
                        <tr class="hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-full {{ $roleColor }}">
                                    {{ $primaryRole }}
                                </span>
                            </td>
