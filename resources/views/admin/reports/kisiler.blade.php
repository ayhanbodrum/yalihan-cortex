...
@section('content')
    ...
    <div class="content-header mb-8">
        ...
        <p class="text-lg text-gray-600 mt-2">Kişi analizi ve performans raporları</p>
    </div>
    ...
    <div class="px-6">
            ...
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-sm text-green-600 font-medium">Aktif Kişi</p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 mb-8">
            ...
            <form method="GET" action="{{ route('admin.reports.kisiler') }}"
                class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Kişi Tipi</label>
                    <select style="color-scheme: light dark;" name="kisi_tipi" class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 w-full transition-all duration-200">
                        <option value="">Tümü</option>
                        <option value="ev_sahibi" {{ request('kisi_tipi') == 'ev_sahibi' ? 'selected' : '' }}>Ev Sahibi
                        </option>
                        <option value="satici" {{ request('kisi_tipi') == 'satici' ? 'selected' : '' }}>Satıcı</option>
                        <option value="alici" {{ request('kisi_tipi') == 'alici' ? 'selected' : '' }}>Alıcı</option>
                        <option value="kiraci" {{ request('kisi_tipi') == 'kiraci' ? 'selected' : '' }}>Kiracı</option>
                        <option value="kiralayan" {{ request('kisi_tipi') == 'kiralayan' ? 'selected' : '' }}>Kiralayan
                        </option>
                        <option value="yatirimci" {{ request('kisi_tipi') == 'yatirimci' ? 'selected' : '' }}>Yatırımcı
                        </option>
                    </select>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold text-gray-800">📈 Kişi Analizi</h2>
                    <div class="flex space-x-2">
                        ...
                        <button onclick="exportToPDF()" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200 focus:ring-2 focus:ring-offset-2 inline-flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-300 bg-white text-gray-700 rounded-lg hover:bg-gray-50 hover:scale-105 active:scale-95 focus:ring-2 focus:ring-gray-500 transition-all duration-200 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700 touch-target-optimized touch-target-optimized">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($customers as $customer)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <x-neo.status-badge :value="ucfirst(str_replace('_', ' ', $customer->kisi_tipi ?? $customer->musteri_tipi ?? 'Belirtilmemiş'))" category="type" />
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                ...
                @else
                    <x-neo.empty-state title="Kişi bulunamadı" description="Seçilen kriterlere uygun kişi bulunmuyor." />
                @endif
            </div>
        </div>
    </div>
