...
<div
    ...
    <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
        <select onchange="changeStatus({{ $gorev->id }}, this.value)"
            class="w-full px-3 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">
            <option value="bekliyor" {{ in_array($gorev->status, ['bekliyor', 'beklemede']) ? 'selected' : '' }}>
                Yapılacaklar</option>
            <option value="devam_ediyor" {{ $gorev->status === 'devam_ediyor' ? 'selected' : '' }}>İşlemde</option>
            <option value="tamamlandi" {{ $gorev->status === 'tamamlandi' ? 'selected' : '' }}>Tamamlandı</option>
        </select>
    </div>
