...
Route::middleware(['auth', 'admin'])->prefix('admin')->group(function () {
    ...
    Route::prefix('api/ai')->name('admin.api.ai.')->group(function () {
        ...
        // Talep Analizi (Mevcut AIController'ınızla uyumlu)
        Route::post('/talep-analiz/{id}', function($id) {
            try {
                $talep = \App\Models\Talep::with(['kisi', 'city', 'ilce'])->findOrFail($id);

                // Mevcut AI servisinizi kullan
        ...
        Route::post('/talep-eslesme/{id}', function($id) {
            try {
                ...
                // Hızlı eşleştirme yap
                $aiService = app(\App\Services\AI\TalepPortfolyoAIService::class);
                $eslesme = $aiService->hizliEsleme($talep);
        ...
        Route::get('/health', function() {
            try {
                ...
                return response()->json([
                    'success' => true,
                    'ai_status' => $health ? 'healthy' : 'unhealthy',
                    'timestamp' => now()->toISOString(),
                    'services' => [
                        'TalepPortfolyoAIService' => $health,
                        'Context7' => true, // Context7 status
                        'Cache' => \Cache::getStore() ? true : false
                    ]
                ]);
