...
class YalihanBekciMCP {
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                },
                {
                    ...
                    description:
                        "Sistem yapısını getirir (model, controller sayıları)",
                    inputSchema: {
                        type: "object",
                        properties: {},
                    },
                },
                {
                    ...
                    description:
                        "Test raporlarından öğrenilmiş hataları gösterir",
                    inputSchema: {
                        type: "object",
                        properties: {
                            limit: {
                                type: "number",
                                description:
                                    "Kaç hata gösterilsin (default: 10)",
                            },
                        },
                    },
                },
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            path: {
                                type: "string",
                                description:
                                    "Kontrol edilecek dizin (default: current)",
                            },
                            excludePaths: {
                                type: "array",
                                items: { type: "string" },
                ...
                },
                {
                    ...
                    description:
                        "Knowledge base dosyalarını birleştirir ve optimize eder",
                    inputSchema: {
                        type: "object",
                        properties: {
                            category: {
                            ...
                            dryRun: {
                                type: "boolean",
                                description:
                                    "Sadece rapor oluştur, değişiklik yapma",
                            },
                        },
                    },
                },
                {
                    ...
                    description:
                        "AI prompt dosyalarını yönetir ve eksikleri tespit eder",
                    inputSchema: {
                        type: "object",
                        properties: {
                            action: {
                            ...
                            promptType: {
                                type: "string",
                                description:
                                    "Prompt tipi (arsa, daire, villa, etc.)",
                            },
                        },
                        required: ["action"],
                    },
    ...
    async validateCode(args) {
        ...
        const violations = context7Rules.checkCode(
            args.code,
    ...
    async learnFromStableCreate() {
        ...
        if (fs.existsSync(stableCreatePath)) {
            ...
            console.error(
                `✅ ${violations.length} yeni kural öğrenildi (stable-create)`
            );
        }
    }
    ...
    async learnFromDocs() {
        ...
        reportFiles.forEach((file) => {
            ...
            if (fs.existsSync(filePath)) {
                ...
                const undefinedVars = content.match(
                    /Tanımsız değişken: \$(\w+)/g
                );
                if (undefinedVars) {
    ...
    async consolidateKnowledge(args) {
        ...
        const consolidationMap = {
            ai: [
                "yalihan-bekci/knowledge/ai-*.json",
                "docs/ai-training/",
                "docs/context7/AI-*.md",
            ],
            context7: [
                "yalihan-bekci/knowledge/context7-*.json",
                "docs/context7/",
                ".context7/",
            ],
            "stable-create": [
    ...
    async manageAiPrompts(args) {
        ...
        const results = {
            ...
            missing_prompts: expectedPrompts.filter(
                (p) => !existingPrompts.includes(p)
            ),
...
`;
                ...
                if (!args.dryRun) {
                    ...
                    fs.writeFileSync(
                        path.join(promptsDir, promptFile),
                        template
                    );
