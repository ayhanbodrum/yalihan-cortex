...
class MigrationCollector {
  ...
  findAllMigrations() {
    ...
    if (fs.existsSync(this.modulesPath)) {
      ...
      modules.forEach(module => {
        const moduleMigrationsPath = path.join(this.modulesPath, module, 'Database/Migrations');
  ...
  async testMigration(migration, rules) {
    ...
    if (rules.enforceModuleStructure && migration.module) {
      ...
      if (moduleNameInFile && moduleNameInFile !== migration.module) {
        result.errors.push(`Migration dosyası ${migration.module} modülünde ama içerikte ${moduleNameInFile} modülü kullanılmış`);
        result.status = 'error';
      }
    }
  ...
  validateVersion(fileName) {
    ...
    if (!versionMatch) {
      errors.push(`Migration dosya adı '${fileName}' YYYY_MM_DD_HHMMSS formatına uygun değil`);
    }
