...
class SeederCollector {
  ...
  async testSeeder(seeder, rules) {
    ...
    if (rules.enforceModuleStructure && seeder.module) {
      ...
      if (moduleNameInFile && moduleNameInFile !== seeder.module) {
        result.errors.push(`Seeder dosyası ${seeder.module} modülünde ama içerikte ${moduleNameInFile} modülü kullanılmış`);
        result.status = 'error';
      }
    }
    ...
    if (content.indexOf('afterLastBatch') === -1) {
      result.warnings.push('Seeder dosyasında afterLastBatch metodu bulunamadı. Migration sonrası otomatik çalıştırma için bu metod gereklidir.');
  ...
  validateDependencies(content) {
    ...
    if (dependsOnMatch) {
      for (const match of dependsOnMatch) {
        const seederClass = match.match(/\$this->call\(\s*([a-zA-Z0-9_\\]+)::class\s*\)/)[1];
  ...
  checkSeederExists(seederClass) {
    ...
    const moduleMatch = seederClass.match(/App\\Modules\\([a-zA-Z0-9_]+)\\Database\\Seeders\\([a-zA-Z0-9_]+)/);
    if (moduleMatch) {
      const [, moduleName, className] = moduleMatch;
      return fs.existsSync(path.join(this.modulesPath, moduleName, 'Database/Seeders', `${className}.php`));
    }
