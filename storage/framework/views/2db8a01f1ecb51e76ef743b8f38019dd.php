...
<?php
    ...
    $passiveEmails = \App\Models\User::where('status', 0)->whereNotNull('email')->limit(5)->pluck('email')->toArray();
?>

<div class="space-y-6">
    
    <div class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-indigo-900/20 dark:via-purple-900/20 dark:to-pink-900/20 rounded-2xl border-2 border-indigo-200 dark:border-indigo-800 p-6 shadow-xl">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br from-indigo-600 to-purple-600 shadow-lg">
                ...
                <div>
                    ...
                    <div class="text-sm text-indigo-700 dark:text-indigo-300">Mükerrer e‑posta ve pasif hesap önerileri</div>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <span class="px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 text-xs font-semibold rounded-full">Context7 Uyumlu</span>
                <?php if(count($duplicateEmails)>0): ?>
                    <span class="px-3 py-1.5 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400 text-xs font-semibold rounded-full">⚠️ Mükerrer: <?php echo e(count($duplicateEmails)); ?></span>
                <?php else: ?>
                    <span class="px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 text-xs font-semibold rounded-full">✅ Temiz</span>
                <?php endif; ?>
                <span class="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400 text-xs font-semibold rounded-full">💤 Pasif: <?php echo e($passiveCount); ?></span>
            </div>
        </div>
        <?php if(count($duplicateEmails)>0 || $passiveCount>0): ?>
            <div class="mt-4 space-y-2 text-xs">
                <?php if(count($duplicateEmails)>0): ?>
                    <div class="flex items-start gap-2 text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg">
                        <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                        </svg>
                        <div>
                            <span class="font-semibold">Mükerrer e-postalar:</span> 
                            <?php echo e(implode(', ', array_slice($duplicateEmails,0,3))); ?><?php if(count($duplicateEmails)>3): ?> ve <?php echo e(count($duplicateEmails)-3); ?> tane daha...<?php endif; ?>
                        </div>
                    </div>
                <?php endif; ?>
                <?php if($passiveCount>0): ?>
                    <div class="flex items-start gap-2 text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg">
                        <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                        </svg>
                        <div>
                            <span class="font-semibold">Pasif hesaplar:</span> 
                            <?php echo e(implode(', ', $passiveEmails)); ?><?php if($passiveCount>count($passiveEmails)): ?> ve <?php echo e($passiveCount-count($passiveEmails)); ?> tane daha...<?php endif; ?>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
    ...
    <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
        ...
        <div class="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center shadow-lg">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">E‑posta</th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">Rol</th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">Durum</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">
                        ...
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">İşlemler</th>
                    </tr>
                </thead>
                <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    <?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold text-sm shadow-lg">
                                    <?php echo e($user->id); ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                <?php
                                    ...
                                    $roleColors = [
                                        ...
                                        'Danisman' => 'bg-gradient-to-br from-blue-100 to-cyan-100 dark:from-blue-900/30 dark:to-cyan-900/30 text-blue-800 dark:text-blue-400 border border-blue-300 dark:border-blue-700',
                                        'Editor' => 'bg-gradient-to-br from-green-100 to-emerald-100 dark:from-green-900/30 dark:to-emerald-900/30 text-green-800 dark:text-green-400 border border-green-300 dark:border-green-700',
                                        'Kullanıcı' => 'bg-gradient-to-br from-gray-100 to-slate-100 dark:from-gray-900/30 dark:to-slate-900/30 text-gray-800 dark:text-gray-400 border border-gray-300 dark:border-gray-700',
                                    ];
                                    $roleColor = $roleColors[$primaryRole] ?? $roleColors['Kullanıcı'];
                                ?>
                                <span class="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-full <?php echo e($roleColor); ?>">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-bold rounded-full 
                                    ...
                                    <span class="w-2 h-2 rounded-full <?php echo e($user->status ? 'bg-green-500 animate-pulse' : 'bg-red-500'); ?>"></span>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Henüz kullanıcı bulunmuyor</h3>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">İlk kullanıcıyı eklemek için "Yeni Kullanıcı" butonuna tıklayın</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
...
// Context7 compliant - Vanilla JS ONLY
console.log('✅ Users page loaded (Tailwind modernized)');
</script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('admin.layouts.neo', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/macbookpro/Projects/yalihanemlakwarp/resources/views/admin/users/index.blade.php ENDPATH**/ ?>