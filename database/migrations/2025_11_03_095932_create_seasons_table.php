...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('seasons', function (Blueprint $table) {
            ...
            // Durum
            $table->boolean('status')->default(true); // Context7: is_active → status
            ...
            $table->index('ilan_id');
            $table->index('type');
            $table->index('status'); // Context7: is_active → status
            $table->index(['ilan_id', 'status']); // Context7: is_active → status
