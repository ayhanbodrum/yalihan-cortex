...
function extractDomainInfo($file, &$categories, &$subcategories, &$types, &$features) {
    ...
    foreach ($lines as $ln => $text) {
        ...
        if (preg_match('/kategori(_id)?["\'\s=>:]/i', $text)) $categories[] = trim($text);
        if (preg_match('/alt[_-]?kategori(_id)?["\'\s=>:]/i', $text)) $subcategories[] = trim($text);
        if (preg_match('/(yayin|yayın|type|tip|ilan_tipi|ilanTipi)["\'\s=>:]/i', $text)) $types[] = trim($text);
        if (preg_match('/ozellik|özellik|feature|property|attribute["\'\s=>:]/i', $text)) $features[] = trim($text);
    }
}
...
foreach ($categorySources as $dir) {
    ...
    foreach ($rii as $file) {
        ...
        if (!in_array($ext, ['php','js','json','md','sql']) && substr($path, -10) !== '.blade.php') continue;
        extractDomainInfo($path, $categories, $subcategories, $types, $features);
    }
}

// 2. Basit çakışma/tutarsızlık tespiti
if (empty($categories)) $problems[] = 'Hiç kategori bulunamadı!';
if (empty($types)) $problems[] = 'Hiç yayın tipi bulunamadı!';
if (empty($features)) $problems[] = 'Hiç özellik bulunamadı!';
...
foreach ($catCounts as $k=>$v) if ($v > 3) $problems[] = "Kategori tekrar/çakışma: $k ($v kez)";
$subcatCounts = array_count_values(array_map('strtolower', $subcategories));
foreach ($subcatCounts as $k=>$v) if ($v > 3) $problems[] = "Alt kategori tekrar/çakışma: $k ($v kez)";
$typeCounts = array_count_values(array_map('strtolower', $types));
foreach ($typeCounts as $k=>$v) if ($v > 3) $problems[] = "Yayın tipi tekrar/çakışma: $k ($v kez)";
$featureCounts = array_count_values(array_map('strtolower', $features));
foreach ($featureCounts as $k=>$v) if ($v > 3) $problems[] = "Özellik tekrar/çakışma: $k ($v kez)";
