...
function smartCalculator() {
    return {
        ...
        async loadCalculationTypes() {
            try {
                const response = await fetch("/admin/api/calculator/types");
                const data = await response.json();

                if (data.success) {
                    this.calculationTypes = data.data;
                }
            } catch (error) {
        ...
        showThemeToast(theme, type) {
            ...
            switch (theme) {
                ...
                case "location":
                    ...
                    break;
                case "features":
                    subtleVibrantToast.features.info(
                        "Özellik Analizi",
                        `${typeName} seçildi`,
                    );
                    break;
                case "media":
                    subtleVibrantToast.media.info(
                        "Gelişmiş Analiz",
                        `${typeName} seçildi`,
                    );
                    break;
                case "system":
                    subtleVibrantToast.system.info(
                        "Sistem Hesaplaması",
                        `${typeName} seçildi`,
                    );
                    break;
        ...
        showErrorToast(theme, typeName, error) {
            switch (theme) {
                case "basic":
                    subtleVibrantToast.basic.error(
                        "Hesaplama Hatası",
                        `${typeName}: ${error}`,
                    );
                    break;
        ...
        async calculate() {
            ...
            try {
                const response = await fetch(
                    "/admin/api/calculator/calculate",
                    {
                        method: "POST",
                        headers: {
                ...
                if (data.success) {
                    ...
                    // Context-aware success toast
                    const theme = this.getCalculationTheme(this.selectedType);
                    const typeName =
                        this.calculationTypes[this.selectedType] ||
                        this.selectedType;
                    ...
                    this.showMessage(
                        "Hesaplama hatası: " + data.error,
                        "error",
                    );

                    // Context-aware error toast
                    const theme = this.getCalculationTheme(this.selectedType);
                    const typeName =
                        this.calculationTypes[this.selectedType] ||
                        this.selectedType;
                    this.showErrorToast(theme, typeName, data.error);
                }
            } catch (error) {
        ...
        getResultLabel(key) {
            ...
            return (
                labels[key] ||
                key.replace("formatted_", "").replace(/_/g, " ").toUpperCase()
            );
        },
        ...
        async loadHistory() {
            try {
                const response = await fetch(
                    "/admin/api/calculator/history?limit=50",
                );
                const data = await response.json();
        ...
        async loadFavorites() {
            try {
                const response = await fetch("/admin/api/calculator/favorites");
                const data = await response.json();
        ...
        async saveToFavorites() {
            ...
            try {
                const response = await fetch(
                    "/admin/api/calculator/favorites",
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                ...
                if (data.success) {
                    ...
                    this.showMessage(
                        "Favori kaydedilemedi: " + data.error,
                        "error",
                    );
                }
            } catch (error) {
                ...
                this.showMessage(
                    "Favori kaydedilirken bir hata oluştu",
                    "error",
                );
            }
        },
        ...
        async removeFavorite(id) {
            if (
                !confirm(
                    "Bu favori hesaplamayı silmek istediğinizden emin misiniz?",
                )
            ) {
                return;
            }
            ...
            try {
                const response = await fetch(
                    `/admin/api/calculator/favorites/${id}`,
                    {
                        method: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": document
