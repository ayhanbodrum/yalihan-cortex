...
class EnhancedFormValidator {
    ...
    setupValidationRules() {
        ...
        this.addValidator('ilan_basligi', [
            ...
            },
            {
                ...
                message: 'İlan başlığında sadece harf, rakam ve temel noktalama işaretleri kullanılabilir'
            }
        ]);
    ...
    async applyRule(rule, value, fieldElement) {
        switch (rule.rule) {
            ...
            case 'server':
                return await this.validateOnServer(rule.endpoint, { value, field: fieldElement?.name });
    ...
    updateFieldUI(fieldName, state, message = '') {
        ...
        if (!successIcon) {
            ...
            successIcon.innerHTML = '<svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>';
